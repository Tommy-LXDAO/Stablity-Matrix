...
//        testResourceParsing();
    ...
    private static void validateTombstoneInfo(TroubleEntity entity) {
        if (entity instanceof AArch64Tombstone) {
            ...
            assert tombstone.getRegisterDumpInfo().getX5() == 0x0L : "X5寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX6() == 0x0L : "X6寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX7() == 0x0L : "X7寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX8() == 0x0L : "X8寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX9() == 0x73682a85L : "X9寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX10() == 0x66642a85L : "X10寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX11() == 0x73622a85L : "X11寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX12() == 0x7c5072d028L : "X12寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX13() == 0x7c5072d048L : "X13寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX14() == 0x7bdab4fcc0L : "X14寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX15() == 0x0L : "X15寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX16() == 0x7d0f7faf50L : "X16寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX17() == 0x7d09b42400L : "X17寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX18() == 0x7bda2d0000L : "X18寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX19() == 0x7c50ab0180L : "X19寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX20() == 0x1L : "X20寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX21() == 0x7c5072d048L : "X21寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX22() == 0x7c5072d028L : "X22寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX23() == 0x7d0f7a87fcL : "X23寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX24() == 0x20L : "X24寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX25() == 0x7bdab51000L : "X25寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX26() == 0x7c50ab0188L : "X26寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX27() == 0x7c50ab01a8L : "X27寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX28() == 0x7d0f7fc000L : "X28寄存器值不匹配";
            assert tombstone.getRegisterDumpInfo().getX29() == 0x7bdab4f9d0L : "X29寄存器值不匹配";
            ...
            assert specialReg.getLr() == 0x7d0f7a7fb8L : "LR寄存器值不匹配";
            assert specialReg.getSp() != null : "SP寄存器为空";
            assert specialReg.getSp() == 0x7bdab4f9a0L : "SP寄存器值不匹配";
            assert specialReg.getPc() != null : "PC寄存器为空";
            assert specialReg.getPc() == 0x7d0f79d8ccL : "PC寄存器值不匹配";
            assert specialReg.getPst() != null : "PST寄存器为空";
            assert specialReg.getPst() == 0x60001000L : "PST寄存器值不匹配";
